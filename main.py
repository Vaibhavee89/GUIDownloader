...
import requests
import os
'https://download-cdn.jetbrains.com/python/pycharm-community-2023.1.2.exe'
class Downloader:
    def __init__(self):
        self.saveto = ""
        self.window = tk.Tk()
        ...
        self.url_entry.pack()
        self.browse_button = tk.Button(text="Browse",
                                       command=self.browse_file)
        self.browse_button.pack()
        self.download_button= tk.Button(text="Download",
    ...
    def browse_file(self):
        saveto = filedialog.asksaveasfilename(initialfile=self.url_entry.get(
            ).split("/")[-1].split("?"[0]))
        self.saveto = saveto
